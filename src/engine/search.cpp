...
SearchResult iter_deepening(Board& b, TranspositionTable& tt, int max_depth){
    ...
    int base_window = 25; // in centipawns
    ...
    for(int depth = 1; depth <= max_depth; depth++){
        ...
        while(true){
            SearchResult r = search_root_window(alpha, beta, b, tt, depth, pv_move.best_move);
...
SearchResult search_root_window(int alpha, int beta, Board& b, TranspositionTable& tt, int depth, Move prev_best){
    SearchResult result;
    result.best_move = 0;
    result.score_cp = 0;
    ...
    if(tt_move && tt_move != prev_best) move_to_index(moves, tt_move, 1);
    const size_t start = (tt_move && tt_move != prev_best && moves.size() > 1) ? 2 : 1;
    if (moves.size() > start) {
        std::stable_partition(moves.begin() + start, moves.end(),
                              [&](Move m){ return is_capture(b, m); });
    }
    ...
    for(Move m : moves) {
        do_move(b, ss, m);
        int score = -alpha_beta_negamax(-beta, -alpha, b, ss, tt, depth - 1);
        undo_move(b, ss, m);
        if(score > best_score){
            best_score = score;
            best_move = m;
...
int alpha_beta_negamax(int alpha, int beta, Board& b, StateStack& ss, TranspositionTable& tt, int depth){
    ...
    // alpha/beta probe may tighten the window so we will keep that
    alpha = alpha_probe;
    beta = beta_probe;

    if(depth == 0) return quiesce(alpha, beta, b, ss);
    std::vector<Move> moves = generate_moves(b, ss);
    ...
    if(tt_move) move_to_index(moves, tt_move, 0); // tt move goes first because its a previously searched best move
    std::stable_partition(moves.begin() + (tt_move ? 1 : 0), moves.end(),
                      [&](Move m){ return is_capture(b, m); }); // captures before noncaptures
